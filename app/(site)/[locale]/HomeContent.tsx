...
/**
 * La portada, entera. Vive fuera de `page.tsx` porque la sirven DOS rutas: `/es`
 * y `/es/studio` · `/es/contact` (ver `[section]/page.tsx`). Las secciones tienen
 * dirección propia pero no página propia, así que el HTML tiene que ser el mismo:
 * si esto estuviera dentro de `page.tsx`, la ruta de sección tendría que repetir
 * la composición y las dos se irían separando con el primer cambio.
 *
  ...
  return (
    <>
      ...
      {/* Estudio y contacto no son páginas propias: se leen aquí, en el orden del menú
          (obra → estudio → contacto), y el menú lleva a su ruta —`/es/studio`— que
          renderiza esta misma portada. Quien entra por el inicio acaba el recorrido
          sin haber cargado nada más. */}
